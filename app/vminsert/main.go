...
func otlpGRPCRequestHandler(w http.ResponseWriter, r *http.Request) bool {
	...
	case opentelemetry.GRPCMetricsServicePath:
		...
		if err := opentelemetry.InsertHandlerForGRPC(r); err != nil {
...
var (
	...
	opentelemetryGRPCPushRequests = metrics.NewCounter(`vm_http_requests_total{path="/opentelemetry.proto.collector.metrics.v1.MetricsService/Export", protocol="opentelemetrygrpc"}`)
	opentelemetryGRPCPushErrors   = metrics.NewCounter(`vm_http_request_errors_total{path="/opentelemetry.proto.collector.metrics.v1.MetricsService/Export", protocol="opentelemetrygrpc"}`)
